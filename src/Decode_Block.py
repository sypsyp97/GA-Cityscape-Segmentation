...
from src.Search_Space import (activation_space, expansion_factor_space,
                              filters_space, head_space, kernel_size_space,
                              normalization_space, residual_space,
                              stride_space, transformer_space)
...
def decoded_block(x, layer_array):
    ...
    # Decode layer configuration from layer_array
    ...
    filters_index = int(
        str(layer_array[5]) + str(layer_array[6]) + str(layer_array[7]), 2
    )
    ...
    # Define a dictionary of supported layer blocks
    layer_type_dict = {
        # Convolutional block
        0: lambda x: conv_block(
            x,
            filters=filters_space[filters_index],
        ...
        1: lambda x: inverted_residual_block(
            x,
            expansion_factor=expansion_factor_space[expansion_factor_index],
            kernel_size=kernel_size_space[kernel_size_index],
        ...
        # MobileViT block
        2: lambda x: mobilevit_block(
            x,
            num_blocks=transformer_space[transformer_index],
    ...
    # Apply the corresponding layer block to the input tensor and return
