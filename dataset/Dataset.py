...
def get_image_and_annotation_dirs(data_split):
    ...
    x_dir = os.path.join(
        "cityscapes", "leftImg8bit_trainvaltest", "leftImg8bit", data_split
    )
    ...
    images_dir, annotations_dir = zip(
        *sorted(
            [
                (x, y.replace("gtFine_color", "gtFine_labelIds"))
                for x, y in zip(
                    glob.glob(os.path.join(x_dir, "*", "*.png")),
                    glob.glob(os.path.join(y_dir, "*", "*gtFine_color.png")),
                )
            ]
        )
    )
...
def read_and_resize_image(img_path, size, is_annotation=False):
    ...
    return (
        tf.keras.utils.to_categorical(resized_img, num_classes=34)
        if is_annotation
        else resized_img
    )
...
val_ds = (
    tf.data.Dataset.from_tensor_slices((x_val, y_val)).batch(batch_size).prefetch(auto)
)
test_ds = (
